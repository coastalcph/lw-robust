...
def get_train_loader(loader, dataset, batch_size,
    ...
    if loader == 'standard':
        if uniform_over_groups is None or not uniform_over_groups:
            ...
            # set to zero in case group is empty
            group_weights[group_weights == float("Inf")] = 0

            # Compute sample weights
            weights = [np.sum([group_weights[idx].numpy() if value else 0.0 for idx, value in enumerate(ex)]) if torch.sum(ex) else np.mean(group_weights.numpy()) for ex in groups]
...
class GroupSampler:
    ...
    def __init__(self, group_ids, batch_size, n_groups_per_batch,
        ...
        # COMMENTED OUT TO SERVE LABEL-WISE GROUPS
        # self.unique_groups, self.group_indices, unique_counts = split_into_groups(group_ids)
        unique_counts = group_ids.sum(0)
        self.unique_groups = [0] * group_ids.size(1)
        self.group_indices = [torch.tensor([sample_idx for sample_idx, group_id in enumerate(group_ids) if group_id[idx] == 1]) for idx in range(len(self.unique_groups))]

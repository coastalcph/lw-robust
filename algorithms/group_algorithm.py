...
class GroupAlgorithm(Algorithm):
    ...
    def update_log(self, results):
        ...
        Args:
            - results (dictionary)
        """
        results = self.sanitize_dict(results, to_out_device='cpu')
        ...
        with torch.no_grad():
            for m in self.logged_metrics:
                if not self.no_group_logging:
                    group_metrics, group_counts, worst_group_metric = m.compute_label_wise(
                        results['y_pred'],
                        results['y_true'],
                        return_dict=False)
            ...
            if 'adv_objective' in self.logged_fields:
                batch_log['Adv-F1'] = self.adv_metric.compute(
                    results['adv_y_pred'],
                    results['g'],
                    return_dict=False).item()
